...
class AgentOrchestratorService:
    def __init__(
        ...
        agent_router_service: AgentRouterService | None = None,
    ...
    def build_draft(
        ...
        local_fields_by_table = {
            table_name: sorted(self._local_field_rule_service.identify_local_fields(schema))
        ...
        return PlanningDraft(
            ...
            agent_run=AgentRunSummary(
                ...
                executed_skills=[
                    AgentSkillExecution("extract_interface_sql", summary=f"{len(interface_infos)} interfaces"),
                    AgentSkillExecution("load_table_schema", summary=f"{len(schemas)} tables"),
                    AgentSkillExecution(
                        "resolve_local_generators",
                        summary=", ".join(f"{table}={len(fields)}" for table, fields in local_fields_by_table.items()),
                    ),
                    AgentSkillExecution("build_table_plans_local", summary=f"{len(local_draft.table_plans)} table plans"),
                ],
            ),
        ...
        return GenerationReport(
            ...
            agent_run=AgentRunSummary(
                ...
                executed_skills=[
                    ...
                    AgentSkillExecution("build_table_plans_local", summary=f"{len(local_draft.table_plans)} table plans"),
                    AgentSkillExecution("sample_table_data", summary=f"{len(sample_rows_by_table)} sampled tables"),
                ],
            ),
            agent_bundle=agent_bundle,
        )
